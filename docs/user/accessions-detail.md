...
- Collection managers can now launch an accession reference merge directly from the References card when at least two references are linked to the accession. The **Merge references** toggle expands an accessible W3.CSS panel with checkboxes to pick candidates, a radio group for the target, and a hand-off to the field-selection step so preferred values are chosen before any merge is applied.
- On the accession row detail page, collection managers and superusers can delete a specimen element from the Elements list. The delete action opens a confirmation page and then returns you to the accession row once the element is removed.
