...
from cms.ocr_processing import (
    classify_specimen_list_page,
    run_specimen_list_raw_ocr,
    run_specimen_list_row_extraction,
)
...
def _set_pipeline_status(page: SpecimenListPage, status: str) -> None:
    if page.pipeline_status in {
        SpecimenListPage.PipelineStatus.IN_REVIEW,
        SpecimenListPage.PipelineStatus.APPROVED,
        SpecimenListPage.PipelineStatus.REJECTED,
    }:
        return
    if page.pipeline_status == status:
        return
    page.pipeline_status = status
...
def run_specimen_list_ocr_queue(
    *,
    limit: int | None = None,
    ids: list[int] | None = None,
    force: bool = False,
    ...
    if force:
        ...
        queryset = SpecimenListPage.objects.filter(
            classification_status=SpecimenListPage.ClassificationStatus.CLASSIFIED,
            pipeline_status__in=[
...
def run_specimen_list_row_extraction_queue(
    *,
    limit: int | None = None,
    ids: list[int] | None = None,
    force: bool = False,
